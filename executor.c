...
int executor(char** args, char** env) {
    ...
    pid = fork();
    if(pid == -1) {
        perror("fork - failed in executor");
        return 1;
    }
...
int child_process(char** args, char** env) {
    ...
    for(int i = 0; path_list[i]; i++) {free(path_list[i]);}
    free(path_string);
    free(path_list);

    //trying to execute the command in cwd
    char* cwd = NULL;
    cwd = getcwd(NULL, 0);
    if(cwd == NULL) {
        perror("getcwd");
...
//splitting PATH string into individual paths
